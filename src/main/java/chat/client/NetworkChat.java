...
public class NetworkChat extends JFrame {
    ...
    public void sendMessage() {
        ...
        if (!trimmedMessage.isEmpty()) {
            if (!login.isEmpty()) {
                saveToFile(trimmedMessage, login);
            }
            message.setText("");
            try {
                        ...
                        try {
                            while (true) {
                                ...
                                if (strFromServer.startsWith("/authok")) {
                                    ...
                                    this.login = strFromServer.split("\\s+")[2];
                                    loadLast100History(login);
                                    break;
                                }
    ...
    private void saveToFile(String trimmedMessage, String login) {
        try (FileWriter fw = new FileWriter("history_" + login + ".txt", true);
             BufferedWriter bw = new BufferedWriter(fw);
             PrintWriter out = new PrintWriter(bw)) {
            out.println(trimmedMessage);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private void loadLast100History(String login) {
        try {
